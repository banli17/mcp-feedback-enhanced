...
class WebFeedbackSession:
    ...
    async def wait_for_feedback(self, timeout: int = 600) -> dict[str, Any]:
        ...
        try:
            ...
            if completed:
                ...
                # 🔴 檢查是否是用戶關閉窗口導致的取消（通過反饋內容判斷）
                if self.feedback_result == "[用戶已關閉反饋窗口]":
                    debug_log(f"會話 {self.session_id} 因用戶關閉窗口而取消")
                    # 返回取消標記，讓 AI 知道用戶取消了操作
                    return {
                        "logs": "\n".join(self.command_logs),
                        "interactive_feedback": self.feedback_result,
                        "images": [],
                        "settings": {},
                        "cancelled": True,  # 添加取消標記
                    }
    ...
    async def cancel_by_window_close(self):
        """
        用戶關閉窗口時自動取消會話
        此方法會立即觸發 feedback_completed 事件，讓等待中的 AI 智能體解除阻塞
        """
        if self.feedback_completed.is_set():
            debug_log(f"會話 {self.session_id} 已完成，跳過窗口關閉處理")
            return

        debug_log(f"⚠️ 會話 {self.session_id} 檢測到用戶關閉窗口，自動取消會話")
        
        # 設置取消標記的反饋內容
        self.feedback_result = "[用戶已關閉反饋窗口]"
        self.status = SessionStatus.COMPLETED
        self.status_message = "用戶取消反饋（關閉窗口）"
        self.last_activity = time.time()
        
        # 立即觸發完成事件，釋放等待中的 wait_for_feedback
        self.feedback_completed.set()
        
        debug_log(f"✅ 會話 {self.session_id} 已自動取消，AI 智能體將收到取消通知")
