...
def process_images(images_data: list[dict]) -> list[ImageContent]:
    ...
    for i, img in enumerate(images_data, 1):
        try:
            ...
            # 創建 MCPImage 並轉換為 ImageContent（MCP 協議原生類型）
            mcp_image = MCPImage(data=image_bytes, format=image_format)
            mcp_images.append(mcp_image.to_image_content())
...
def get_default_timeout() -> int:
    ...
    支援的環境變量（優先順序從高到低）：
    - MCP_TIMEOUT: 直接指定超時秒數
    - MCP_DEFAULT_TIMEOUT: 別名
    
    Returns:
        int: 超時時間（秒），默認 600
    """
    # 嘗試從環境變量讀取
    timeout_str = os.getenv("MCP_TIMEOUT") or os.getenv("MCP_DEFAULT_TIMEOUT")
    
    if timeout_str:
        try:
            timeout = int(timeout_str)
            if timeout > 0:
                debug_log(f"使用環境變量設定的超時時間: {timeout} 秒")
                return timeout
            else:
                debug_log(f"環境變量超時時間無效 ({timeout})，使用默認值 600")
        except ValueError:
            debug_log(f"環境變量超時時間格式錯誤 ({timeout_str})，使用默認值 600")
...
async def interactive_feedback(
    ...
    timeout: Annotated[int, Field(description="等待用戶回饋的超時時間（秒）")] = 0,
) -> list:
    ...
    Args:
        ...
        timeout: Timeout in seconds for waiting user feedback (default: from MCP_TIMEOUT env or 600 seconds)

    Returns:
        list: List containing TextContent and MCPImage objects representing user feedback
    """
    # 🔴 處理 timeout 參數：如果為 0 或未指定，使用環境變量或默認值
    if timeout <= 0:
        timeout = _DEFAULT_TIMEOUT
        debug_log(f"使用默認超時時間: {timeout} 秒")
    else:
        debug_log(f"使用指定超時時間: {timeout} 秒")
